...
const defaultContext = {
  users: [],
  currentUser: null,
  register: (_username, _password) => ({ ok: false, message: "Not implemented" }),
...
export function UserProvider({ children }) {
  ...
  const login = useCallback((username, password) => {
    ...
    const found = users.find(
      (u) => u.username.toLowerCase() === normalized.toLowerCase() && u.password === String(password)
    );
    if (!found) {
      return { ok: false, message: "Username atau kata sandi salah" };
    }
    setCurrentUser({ username: found.username });
    return { ok: true, message: "Berhasil masuk", user: { username: found.username } };
  }, [users]);
  ...
  const removeUser = useCallback((username) => {
    ...
    setUsers((prev) => prev.filter((u) => u.username.toLowerCase() !== normalized));
    setCurrentUser((prev) => (prev && prev.username.toLowerCase() === normalized ? null : prev));
    return { ok: true, message: "Pengguna dihapus" };
  }, [users]);

  const value = useMemo(() => ({ users, currentUser, register, login, logout, removeUser }), [users, currentUser, register, login, logout, removeUser]);
